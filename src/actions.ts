...
export class SigmaFixer implements vscode.CodeActionProvider {
    ...
    public provideCodeActions(
        ...
        if (context.diagnostics[0].code === "sigma_containsMiddle") {
            ...
            const fixContains = this.createReplaceFix(
                document,
                context.diagnostics[0].range,
                newtext,
                "Move 'contains' to end.",
            )
            let newRange = new vscode.Range(
                document.lineAt(context.diagnostics[0].range.start.line).lineNumber,
                context.diagnostics[0].range.start.character,
                document.lineAt(range.start.line).lineNumber,
                range.start.character + "contains|".length,
            )
        ...
        if (context.diagnostics[0].code === "sigma_trailingWhitespace") {
            // "     $"
            const removeWhitespaces = this.createReplaceFix(
                document,
                context.diagnostics[0].range,
                "",
                "Remove Whitespaces",
