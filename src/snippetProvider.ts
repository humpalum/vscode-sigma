...
function generateUUIDSnippet(snippet: vscode.SnippetString = new vscode.SnippetString(), numTabs = 0): vscode.SnippetString {
    if (debug) { console.log("SigmaSnippetCompletionItemProvider: Generating 'UUID' snippet"); }
    const tabs = '\t'.repeat(numTabs);
    snippet.appendText(`id: `);
    snippet.appendVariable('UUID', "Could not generate UUID");
    return snippet;
}
function generateStatusSnippet(snippet: vscode.SnippetString = new vscode.SnippetString(), numTabs = 0): vscode.SnippetString {
    if (debug) { console.log("SigmaSnippetCompletionItemProvider: Generating 'status' snippet"); }
    snippet.appendText(`status: `);
    snippet.appendChoice(['experimental', 'test', 'stable', 'deprecated', 'unsupported']);
    return snippet;
}
function generateAuthorSnippet(snippet: vscode.SnippetString = new vscode.SnippetString(), numTabs = 0): vscode.SnippetString {
    if (debug) { console.log("SigmaSnippetCompletionItemProvider: Generating 'author' snippet"); }
...
function generateDateSnippet(snippet: vscode.SnippetString = new vscode.SnippetString(), numTabs = 0): vscode.SnippetString {
    if (debug) { console.log("SigmaSnippetCompletionItemProvider: Generating 'date' snippet"); }
    snippet.appendText('date: ');
    generateTodaySnippet(snippet);
    return snippet;
}

function generateModifiedSnippet(snippet: vscode.SnippetString = new vscode.SnippetString(), numTabs = 0): vscode.SnippetString {
    if (debug) { console.log("SigmaSnippetCompletionItemProvider: Generating 'date' snippet"); }
    snippet.appendText('modified: ');
    generateTodaySnippet(snippet);
    return snippet;
}
function generateTodaySnippet(snippet: vscode.SnippetString = new vscode.SnippetString(), numTabs = 0): vscode.SnippetString {
    if (debug) { console.log("SigmaSnippetCompletionItemProvider: Generating 'today' snippet"); }
...
function generateLevelSnippet(snippet: vscode.SnippetString = new vscode.SnippetString(), numTabs = 0): vscode.SnippetString {
    if (debug) { console.log("SigmaSnippetCompletionItemProvider: Generating 'level' snippet"); }
...
function generateRuleSnippet(snippet: vscode.SnippetString = new vscode.SnippetString(), numTabs = 0): vscode.SnippetString {
    if (debug) { console.log("SigmaSnippetCompletionItemProvider: Generating 'newRule' snippet"); }
...
export class SigmaSnippetCompletionItemProvider implements vscode.CompletionItemProvider {
    private generateBasicItems(): vscode.CompletionList {
        ...
        const newRuleItem: vscode.CompletionItem = new vscode.CompletionItem('newrule', vscode.CompletionItemKind.Snippet);
        ...
        const newTitleItem: vscode.CompletionItem = new vscode.CompletionItem('title: ', vscode.CompletionItemKind.Snippet);
        ...
        const statusItem: vscode.CompletionItem = new vscode.CompletionItem('status: ', vscode.CompletionItemKind.Snippet);
        ...
        const authorItem: vscode.CompletionItem = new vscode.CompletionItem('author: ', vscode.CompletionItemKind.Snippet);
        authorItem.detail = 'Generate a new author (sigma)';
        authorItem.insertText = new vscode.SnippetString('author: ');
        authorItem.documentation = new vscode.MarkdownString('Generates Author. \nSet your name in the sigma.Author config');
        ...
        const modifiedDateItem: vscode.CompletionItem = new vscode.CompletionItem('modified: ', vscode.CompletionItemKind.Snippet);
        ...
        const logsourceItem: vscode.CompletionItem = new vscode.CompletionItem('logsource: ', vscode.CompletionItemKind.Snippet);
        ...
        const detectionItem: vscode.CompletionItem = new vscode.CompletionItem('detection: ', vscode.CompletionItemKind.Snippet);
    ...
    public provideCompletionItems(doc: vscode.TextDocument, pos: vscode.Position, token: vscode.CancellationToken): vscode.ProviderResult<vscode.CompletionList> {
        return new Promise((resolve, reject) => {
            token.onCancellationRequested(() => {
                if (debug) { console.log('SigmaSnippetCompletionItemProvider: Task cancelled!'); }
                resolve(undefined);
            });
            try {
